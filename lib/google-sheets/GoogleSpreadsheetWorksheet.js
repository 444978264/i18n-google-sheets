...
import { columnToLetter, getFieldMask, letterToColumn } from "./utils"
...
function checkForDuplicateHeaders(headers) {
  ...
  _.each(checkForDupes, (grouped, header) => {
    ...
    if (grouped.length > 1) {
      throw new Error(
        `Duplicate header detected: "${header}". Please make sure all non-empty headers are unique`
      )
    }
  })
}
...
export class GoogleSpreadsheetWorksheet {
  ...
  _ensureInfoLoaded() {
    if (!this._rawProperties) {
      throw new Error(
        "You must call `doc.loadInfo()` again before accessing this property"
      )
    }
  }
  ...
  get colCount() {
    throw new Error(
      "`colCount` is deprecated - please use `columnCount` instead."
    )
  }
  set rowCount(newVal) {
    ...
    filtersArray = _.map(filtersArray, (filter) => {
      ...
      if (_.isObject(filter)) {
        ...
        if (filter.sheetId !== this.sheetId) {
          throw new Error(
            "Leave sheet ID blank or set to matching ID of this sheet"
          )
        } else {
          return filter
        }
        ...
        throw new Error(
          "Each filter must be a A1 range string or gridrange object"
        )
      }
    })
  ...
  async saveCells(cellsToUpdate) {
    ...
    const responseRanges = _.map(
      cellsToUpdate,
      (c) => `${this.a1SheetName}!${c.a1Address}`
    )
  ...
  async loadHeaderRow(headerRowIndex) {
    ...
    const rows = await this.getCellsInRange(
      `A${this._headerRowIndex}:${this.lastColumnLetter}${this._headerRowIndex}`
    )
    if (!rows) {
      throw new Error(
    ...
    if (!_.compact(this.headerValues).length) {
      throw new Error(
        "All your header cells are blank - fill the first row with header values before trying to interact with rows"
      )
    }
  ...
  async clearRows(options = {}) {
    ...
    await this._spreadsheet.axios.post(
      `/values/${this.encodedA1SheetName}!${startRowIndex}:${endRowIndex}:clear`
    )
  }
  ...
  async insertDimension(columnsOrRows, range, inheritFromBefore = null) {
    ...
    if (!_.isObject(range))
      throw new Error(
        "`range` must be an object containing `startIndex` and `endIndex`"
      )
    ...
    if (inheritFromBefore && range.startIndex === 0) {
      throw new Error(
        "Cannot set inheritFromBefore to true if inserting in first row/column"
      )
    }
  ...
  async clear(a1Range) {
    ...
    await this._spreadsheet.axios.post(
      `/values/${this.encodedA1SheetName}${range}:clear`
    )
    this.resetLocalCache(true)
  }
  async downloadAsCSV(returnStreamInsteadOfBuffer = false) {
    return this._spreadsheet._downloadAs(
      "csv",
      this.sheetId,
      returnStreamInsteadOfBuffer
    )
  }
